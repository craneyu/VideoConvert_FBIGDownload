...
- [x] 5.2 [P] 重寫 docs/windows-verification.md 的 J 節，使其從「驗證現況是否如預期（尚未實作）」改為「驗證自動判斷是否正確」，並保留既有的實測數據作為對照基準。需等目前正在審查的文件 PR 合併後再進行，避免同檔衝突。驗證：內容審閱，確認 J 節不再宣稱 Windows 尚未實作，且步驟涵蓋有／無解碼能力兩種環境。
