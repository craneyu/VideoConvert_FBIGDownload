...
- [ ] 4.1 讓下載佇列的並行判斷改讀網路並行度設定，移除前端的固定常數，使調高設定後立即有更多待處理下載開始。交付 `Concurrent Download Limit`，並落實 design 決策「網路並行度留在前端，Rust 只擁有 CPU 許可池」。實作位置為 `src/routes/+page.svelte` 的佇列推進邏輯。驗證：手動把設定由 2 改為 4，在 2 個下載進行中且 3 個待處理的狀態下，確認另外 2 個待處理下載開始。
